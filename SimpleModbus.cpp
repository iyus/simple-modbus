...
/*--Cloned from SimpleModbus.h */
/*
  Add Function 06 (Write single register)
*/
                ...
                //  crcLo, crcHi
				  ...
				  */
				  else if (function == 6)
				  {
					  if (startingAddress < holdingRegsSize) // check exception 2 ILLEGAL DATA ADDRESS
					  {
							 unsigned int dreg = no_of_registers; 
                // ID, function, adrr, (dataLo + dataHi)*number of registers,
                //  crcLo, crcHi
							  unsigned char responseFrameSize = 8; 
							  frame[0] = slaveID;
							  frame[1] = function;
							  index = startingAddress;
							  regs[index]=	dreg; // simpan di buffer reg modbus		
							  crc16 = calculateCRC(responseFrameSize - 2);
							  frame[responseFrameSize - 2] = crc16 >> 8; // split crc into 2 bytes
							  frame[responseFrameSize - 1] = crc16 & 0xFF;
							  sendPacket(responseFrameSize);
